...
interface ChecklistFillFormProps {
  complianceId: string;
  checklistId: string;
}

export default function ChecklistFillForm({ complianceId, checklistId }: ChecklistFillFormProps) {
      ...
      try {
        ...
        if (responseData && !responseError) {
          ...
          if (responseData.response_data) {
            // Set form values
            Object.entries(responseData.response_data).forEach(([key, value]) => {
              if (value && typeof value === 'object' && 'type' in value && value.type === 'document' && 
