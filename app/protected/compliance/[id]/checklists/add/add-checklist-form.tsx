...
export default function AddChecklistForm({ complianceId, frameworkName }: AddChecklistFormProps) {
  const router = useRouter();
  
  // Initial state with one sample item
  const [title, setTitle] = useState("Sample Checklist");
  const [description, setDescription] = useState("A sample compliance checklist");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await addChecklist(formData);
  ...
  return (
    <div className="flex-1 w-full flex flex-col gap-8">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold">Add Checklist - {frameworkName}</h1>
      </div>
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Build your checklist using the visual editor below.
              </p>
            </CardHeader>
