...
export default async function ViewComplianceChecklistsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <DashboardLayout userProfile={currentUserProfile}>
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-sky-50 via-blue-50 to-indigo-50 rounded-xl p-6 border border-sky-200">
          <h1 className="text-2xl font-bold text-sky-900">{compliance.name} - Checklists</h1>
          <p className="text-sky-600 mt-1">View and manage checklists</p>
        </div>
          ...
          {checklists?.map((checklist) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-semibold text-sky-900">Checklist #{checklist.id}</h3>
